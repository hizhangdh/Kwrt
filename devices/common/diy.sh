...
sed -i '$a src-git kiddin9 https://github.com/hizhangdh/op-packages.git;main' feeds.conf.default
...
wget-ssl curl autocore htop nano zram-swap kmod-lib-zstd kmod-tcp-bbr bash openssh-sftp-server block-mount resolveip luci-app-filemanager luci-app-wifihistory /" include/target.mk

sed -i "s/^.*vermagic$/\techo '1' > \$(LINUX_DIR)\/.vermagic/" include/kernel-defaults.mk

status=$(curl -H "Authorization: token $REPO_TOKEN" -s "https://api.github.com/repos/hizhangdh/op-packages/actions/runs" | jq -r '.workflow_runs[0].status')
echo "$status"
...
sed -i "s/OpenWrt/Mi-R3D/g" package/base-files/files/bin/config_generate package/base-files/image-config.in package/network/config/wifi-scripts/files/lib/wifi/mac80211.uc config/Config-images.in Config.in include/u-boot.mk include/version.mk || true
